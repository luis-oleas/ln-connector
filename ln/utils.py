...
def set_data_nodes_edges(data: nx, is_message: bool = True) -> nx:
    ...
    for node in data['nodes']:
        ...
        if 'alias' not in node: node['alias'] = node['pub_key'][:4] + '..' + node['pub_key'][-4:]
...
def get_parameters_connection(parameters: dict, g1: nx) -> dict:
    ...
    for n in g1.nodes(data=True):
        ...
        if validate_dir(macaroon) and validate_dir(cert):
            ...
            macaroon = replace_path_os(parameters["polar_path"] + 'c-lightning/' + n[1]['alias'] +
                                       parameters["connector"]["c-lightning"]["macaroon_dir"])
            if validate_dir(macaroon):
                result = check_params_connector("c-lightning", length, "", macaroon)

                connectors["c-lightning"][n[0]] = {"macaroon_dir": macaroon, "info": result[1]}
            else:
...
def check_params_connector(implementation: str, length: int, host: str, macaroon: str = None, cert: str = None,
                           user: str = None, passwd: str = None, pub_key: str = None) -> Tuple[int, Optional[Any]]:
    """
    ...
    if implementation == "lnd":
        ...
        if implementation == "c-lightning":
            result = clight.get_info(macaroon)
        else:
