...
class LNConnector:
    ...
    def __init__(self, json_filename_temp: str, implementation: dict = None, balance: dict = None, htlc: dict = None):
        ...
        while True:
            try:
                ...
                self.__set_param_node(is_snapshot=False)
                # Function to load g1 and g2 based on the network connected
                data = lnd.describe_graph(self.macaroon, self.secure_channel, True)
        ...
        if payment.error is None:
            ...
            for h in payment.routes[0].hops:
                ...
                if label_edge in self.edgeDict:
                    ...
                    print('%s%s%sAMOUNT: %s - FEE: %s' % (utils.spaces, utils.spaces, utils.spaces, h.amt_2_fwrd, h.fee))
                    htlc = route_pay.HTLC(
                    ...
                    pending = route_pay.PendingHtlc(incoming=False, hash_lock=payment_hash,
                                                    amount=htlc.htlc_payment.hop.amt_2_fwrd + 2 * htlc.htlc_payment.hop.fee,
                    ...
                    if label_edge in self.edgeDict:
                        ...
                        for pvt in e[label_edge]['htlc']:
                            ...
                            if htlc['payment_preimage'] is not None and htlc['payment_hash'] == payment.payment_hash \
                                ...
                                print('%sUNBLOCK ON THE CHANNEL_ID: %s FROM %s TO %s' % (utils.spaces, label_edge,
                                                                                         self.nodeDict[edge[1]]['alias'],
                                                                                         self.nodeDict[edge[0]]['alias']))
                                ...
                                print('%s%sPAYMENT ON THE CHANNEL_ID: %s FROM %s TO %s' % (utils.spaces, utils.spaces,
                                                                                           opposite_label_edge,
                                                                                           self.nodeDict[edge[0]]['alias'],
                                                                                           self.nodeDict[edge[1]]['alias']))
        ...
        if self.is_manual_test == "y":
            while input("REQUEST A NEW PAYMENT (y/n)?\n") == 'y':
                ...
                while input("REQUEST A NEW ROUTE (y/n)?\n") == 'y':
                    payment = lnd.query_routes(self.macaroon, self.g1, self.secure_channel,
    ...
    def update_snapshot_node(self):
        snapshot = self.parameters["snapshot"]
            ...
            if key == "lnd" and value["flag"]:
                ...
                for route in value["routes"]:
                    for i in range(self.parameters["loop"]):
                        payments[str(index.preinc())] = lnd.query_routes(self.macaroon, self.g1,
                                                                         self.secure_channel,
...
ln_graph = LNConnector(json_filename, balance=balance_config, htlc=htlc_config)

if hasattr(LNConnector, 'g1') and hasattr(LNConnector, 'g2'):
